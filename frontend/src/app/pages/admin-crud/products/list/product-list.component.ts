...
export class ProductList implements OnInit {
    ...
    async exportCSV() {
        try {
            ...
            const initialData = await this.productService.getProducts(initialParams).toPromise();
            ...
            for (let page = 0; page < totalPages; page++) {
                ...
                const data = await this.productService.getProducts(params).toPromise();
                if (data?.list) {
                    allProducts = [...allProducts, ...data.list];
                }
            }
