...
interface IAdvertisementProduct {
    id_advertisement?: string;
    id_product?: string;
...
interface IAdvertisementHistory {
    id_advertisement: string;
    dt_history: Date;
    st_status: string;
    st_action: string;
...
export class AdvertisementManager {
    ...
    async addHistory(data: IAdvertisementHistory): Promise<void> {
        const query = `
            ...
            ) VALUES ($1, $2, $3, $4, $5, 'crawler', CURRENT_TIMESTAMP)
        `;
        
        const values = [
            data.id_advertisement, data.dt_history,
            data.st_status, data.st_action, data.st_history
        ];
    ...
    async updateAdvertisementStatus(id: string, status: string): Promise<void> {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            ...
            await client.query(`
                ...
                SET st_status = $1, st_updated_by = 'crawler', dt_updated = CURRENT_TIMESTAMP
                WHERE id_advertisement = $2
            `, [status, id]);

            // Adiciona um registro no histórico
            await client.query(`
                INSERT INTO tb_advertisement_history (
                    id_advertisement, dt_history, st_status, st_action, st_created_by, dt_created
                ) VALUES ($1, NOW(), $2, 'STATUS_UPDATE', 'crawler', CURRENT_TIMESTAMP)
            `, [id, status]);
    ...
    async updateAdvertisement(record: IAdvertisement, data: IAdvertisement): Promise<void> {
        ...
        try {
            ...
            await client.query(`
                INSERT INTO tb_advertisement_history (
                    id_advertisement, dt_history, st_status, st_action, st_history, st_created_by, dt_created
                ) VALUES ($1, NOW(), $2, 'CRAWLER_UPDATE', $3, 'crawler', CURRENT_TIMESTAMP)
            `, [record.id_advertisement, record.st_status, JSON.stringify(data)]);
