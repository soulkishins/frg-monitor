...
export class AdvertisementService {
    ...
    public async saveAdvertisements(params: IKeyword, ads: IAdvertisement[]): Promise<void> {
        try {
            ...
            if (ads.length > 0) {
                console.log('saveAdvertisements: Processando página', ads.length);
                console.log('saveAdvertisements: Anúncios', ads);
                params.statistics.nr_pages += this.getPages(ads.length); // Incrementa o número de páginas processadas
                params.statistics.nr_total += ads.length; // Incrementa o número total de anúncios localizados
                await this.processAdvertisements(ads, params); // Processa os anúncios da página atual
            }
            ...
            // Atualiza o status da execução
            console.log('saveAdvertisements: Atualizando estatisticas', params.statistics);
            await this.adManager.updateStatistics(params);
        }
    }

    private getPages(ads: number): number {
        const pagesFloat = ads / 60;
        const pages = Math.ceil(pagesFloat);
        if (pages < pagesFloat) {
            return pages + 1;
        }
        return pages;
    ...
    private async processAdvertisements(
        ...
        let sqsAds: (string|undefined)[] = await Promise.all(ads.map(async(advertisement) => {
            ...
            if (!existingAd || advertisement.st_status !== 'INVALIDATE') {
                ...
                await this.processAdvertisement(advertisement, existingAd, params);
                ...
                if (advertisement.st_status !== 'ERROR') {
                    return advertisement.id_advertisement;
                }
            }
            return undefined;
        }));
